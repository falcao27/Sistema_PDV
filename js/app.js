...
function renderNavegacao() {
    ...
    const podeCriarPedido = ['painel', 'pedidos', 'mesas'].includes(viewAtual);
...
function renderPainel() {
    ...
    $('#painelView').innerHTML = `
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            ${metricCard('Pedidos ativos', ativos, 'Cozinha e salao', 'bg-white dark:bg-gray-800')}
            ${metricCard('Itens criticos', criticos.length, 'Estoque baixo', 'bg-yellow-200')}
        </div>
        ...
        <article class="floating-card bg-gray-50 dark:bg-gray-900 border dark:border-gray-700 rounded-lg p-4">
            <div class="flex items-start justify-between gap-3 mb-3">
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">${pedido.criadoPor?.nome || 'Nao informado'}</p>
                </div>
    ...
    $('#mesasView').innerHTML = `
        <div class="premium-card bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                <h3 class="text-lg font-bold dark:text-white">Painel de mesas</h3>
                <button id="cadastrarMesaBtn" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">
                    <i class="fas fa-plus mr-2"></i>Cadastrar nova mesa
                </button>
            </div>
...
function renderMesaCard(mesa) {
    ...
    const statusClass = aberta ? 'text-green-600 dark:text-green-300 font-bold' : 'text-gray-500 dark:text-gray-300';
    const cardHoverClass = 'transition-transform transition-shadow duration-200 hover:-translate-y-1 hover:shadow-xl';
    const acoes = aberta ? `
                <button class="bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white px-3 py-2 rounded" data-editar-mesa="${mesa.id}"><i class="fas fa-edit mr-1"></i>Editar</button>
                <button class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded" data-add-pedido="${mesa.id}"><i class="fas fa-plus mr-1"></i>Adicionar pedido</button>
                <button class="bg-indigo-500 hover:bg-indigo-600 text-white px-3 py-2 rounded" data-ver-pedidos-mesa="${mesa.id}"><i class="fas fa-list mr-1"></i>Pedidos abertos</button>
                <button class="bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded" data-excluir-mesa="${mesa.id}"><i class="fas fa-trash mr-1"></i>Excluir</button>
                <button class="bg-green-500 hover:bg-green-600 text-white px-3 py-2 rounded col-span-2" data-fechar-mesa="${mesa.id}"><i class="fas fa-receipt mr-1"></i>Fechar</button>
    ` : `
                <button class="bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white px-3 py-2 rounded" data-editar-mesa="${mesa.id}"><i class="fas fa-edit mr-1"></i>Editar</button>
                <button class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded" data-add-pedido="${mesa.id}"><i class="fas fa-plus mr-1"></i>Adicionar pedido</button>
                <button class="bg-indigo-500 hover:bg-indigo-600 text-white px-3 py-2 rounded" data-ver-pedidos-mesa="${mesa.id}"><i class="fas fa-list mr-1"></i>Pedidos abertos</button>
    ...
    return `
        <article class="${cardHoverClass} floating-card border ${aberta ? 'border-green-500 dark:border-green-400' : 'dark:border-gray-700'} rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
            <div class="flex justify-between items-start gap-3 mb-3">
                <div>
                    <h4 class="text-xl font-bold dark:text-white">${mesa.nome}</h4>
                    <p class="text-sm ${statusClass}">${mesa.status} - ${pedidosMesa.length} pedido(s)</p>
                </div>
                <strong class="dark:text-white">${money(total)}</strong>
            </div>
...
function renderDelivery() {
    $('#deliveryView').innerHTML = `
        <div class="premium-card bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <h3 class="text-lg font-bold mb-2 dark:text-white">Delivery</h3>
            <p class="text-gray-600 dark:text-gray-300">Modulo reservado para integracao com API do iFood e apps de lanches.</p>
        </div>
    `;
}
...
function confirmarPedido() {
    ...
    state.pedidos.push({
        ...
        mesaId: mesaAtual,
        itens: carrinhoAtual,
        total: carrinhoAtual.reduce((soma, item) => soma + item.preco * item.quantidade, 0),
        status: 'aguardando',
