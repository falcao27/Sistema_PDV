...
export function adicionarAoCarrinho(produto, quantidade, numeroMesa) {
    ...
    if (!mesa) {
        alert('Abra ou selecione uma mesa antes de adicionar produtos.');
        return;
    }
...
export function renderTabs() {
    ...
    Object.keys(mesas).forEach(numeroMesa => {
        ...
        tabButton.className = 'tab-button px-4 py-2 rounded-t-lg focus:outline-none';
...
export function abaterValorParcial(numeroMesa, valor) {
    const mesa = mesas[numeroMesa];
    if (!mesa) {
        alert('Mesa nao encontrada para pagamento parcial.');
        return;
    }
...
export function finalizarPedido(numeroMesa, metodoPagamento, observacoes, incluirServico) {
    const mesa = mesas[numeroMesa];
    if (!mesa) {
        alert('Mesa nao encontrada para finalizar o pedido.');
        return;
    }
